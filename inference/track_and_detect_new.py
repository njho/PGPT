...
class Track_And_Detect(object):
    ...
    def __init__(
        self, gpu_id=0, track_model=None, pose_model=None, embedding_model=None
    ):
        print(gpu_id)
    ...
    def update_id(self, frame, rgb_frame, det_bbox, track_id):
        ...
        former_frame_flag, former_feature = former_track_dict["frame_flag"], np.array(
            former_track_dict["feature"]
        )
    ...
    def match_detection_tracking_oks_iou_embedding(self, detections, track_list, frame):
        ...
        for database_id in self.track_id_dict:
            database_id_bbox = self.track_id_dict[database_id]["bbox_and_score"] + [
                database_id
            ]
        ...
        (
            ...
            unmatched_trackers,
        ) = self.matcher.associate_detections_to_trackers_iou(
            detections, database_bbox_list, iou_threshold=self.iou_match_thresh
        )
        ...
        (
            ...
            embedding_unmatched_trackers,
        ) = self.matcher.associate_detections_to_trackers_embedding(
            det_feature_list,
            track_feature_list,
        ...
        for new_index in embedding_unmatched_detections:
            ...
            if (
                det_score >= self.effective_detection_thresh
                and np.sum(pose_value >= self.effective_keypoints_thresh)
