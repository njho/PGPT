...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, cfg, flag=2, **kwargs):
        ...
        self.embedding_layer = nn.Sequential(
            self._make_embedding_layer(block, 64, 1, stride=1),
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
        )

        self.pose_conv = nn.Sequential(
            nn.Conv2d(17, 256, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(256),
        ...
        self.fc_feature = nn.Linear(
            256 * int(self.input_height / 32) * int(self.input_width / 32), 2048
        )
    ...
    def _make_embedding_layer(self, block, planes, blocks, stride=1):
        downsample = nn.Sequential(
            nn.Conv2d(
                2048, planes * block.expansion, kernel_size=1, stride=stride, bias=False
            ),
            nn.BatchNorm2d(planes * block.expansion, momentum=BN_MOMENTUM),
        )
    ...
    def forward(self, x, adj, flag=0):
        ...
        if flag == 0:
            ...
            align_feature = align_feature.type(torch.FloatTensor)
