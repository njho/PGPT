...
def get_equality_matrix(a, b):
    ...
    m = len(a)
    n = len(b)
    return np.repeat(np.expand_dims(a, axis=1), n, axis=1) == np.repeat(
        np.expand_dims(b, axis=0), m, axis=0
    )


def add_images(writer, img, boxes):
    writer.add_image(
        "Image_pre", vutils.make_grid(img, nrow=2, normalize=True, scale_each=True)
    )
...
def show_circle(img, boxes, valid):
    ...
    for i in range(batch_size):
        ...
        for b in boxes1:
            img1 = cv2.circle(
                img1,
                tuple(((b + 1) / 2.0 * config["sst_dim"]).astype(int)),
                20,
                [255, 0, 0],
                thickness=3,
            )

        img1 = torch.from_numpy(img1.astype(np.float) - config["mean_pixel"]).permute(
            2, 0, 1
        )
        images.append(img1)
...
def getProperbility(input, target, mask0, mask1):
    ...
    if mask_pre_num:
        accuracy = (
            indexes[mask_pre][:-1] == indexes_[mask_pre][:-1]
...
def show_matching_circle(
    img_pre, img_next, boxes_pre, boxes_next, indexes_pre, indexes_next
):
    H = config["sst_dim"]
    W = H
    ...
    for i in range(len(indexes_pre)):
        ...
        if (
            i_next.data[0] >= config["max_object"]
            or i_pre.data[0] >= config["max_object"]
        ):
            continue
...
def show_matching_rectangle(
    img_pre, img_next, boxes_pre, boxes_next, labels, show_matching=True
):
    img_p = img_pre.copy()
    img_n = img_next.copy()
    for box in boxes_pre[:, 0:4]:
...
def show_batch_circle_image(
    ...
    for i in range(batch_size):
        ...
        boxes1 = boxes_pre[i, :, 0, 0, :2].data.cpu().numpy()
        boxes1 = boxes1[valid1 == 1]
        img1 = np.clip(img1, 0, 255).astype(np.uint8).copy()
        ...
        boxes2 = boxes_next[i, :, 0, 0, :2].data.cpu().numpy()
        ...
        if "save_images_folder" in config and iteration != -1:
            cv2.imwrite(
                os.path.join(
                    config["save_images_folder"], "{0:06}.png".format(iteration)
                ),
                img,
            )
